                    ...
                    if let Some(exemption_rules) = config_json.get("exemptionRules") {
                        if let Some(exemption_array) = exemption_rules.as_array() {
                            for exemption_entry in exemption_array {
                                ...
                                // 如果有path_patterns就添加规则（host_patterns可以为空）
...
impl SpHttpContext {
    ...
    /// 从Kubernetes环境变量自动检测service_name
    fn detect_service_name(&self, config_service_name: &str) -> String {
        log::error!("SP: ========== 开始服务名检测过程 ==========");
        
        // 首先打印所有环境变量
       // self.log_all_environment_variables();
        
        // 然后打印所有可用的Envoy属性
        //self.log_all_envoy_attributes();
        
        // 优先级顺序：
        // 1. 配置文件中的service_name（如果不是默认值）
        // 2. Envoy node metadata中的WORKLOAD_NAME
        // 3. Envoy node metadata中的app标签
        // 4. Envoy node metadata中的NAME
        // 5. source.workload.name（对于入站流量）
        // 6. destination.workload.name（对于出站流量）
        // 7. destination.service.name
        // 8. cluster_name解析
        // 9. Kubernetes Service Account名称
        // 10. Pod名称 (HOSTNAME 或 POD_NAME)
        // 11. SERVICE_NAME环境变量
        // 12. 特定的服务标识请求头（x-service-name等）
        // 13. 路由和集群元数据
        // 14. Istio特定属性
        // 15. xDS配置信息
        // 16. 环境相关属性
        ...
        log::error!("SP: 配置的service_name: '{}'", config_service_name);

        // 1. 如果配置文件中有非默认的service_name，直接使用
        log::error!("SP: [步骤1] 检查配置文件中的service_name");
        if !config_service_name.is_empty() && config_service_name != "default-service" {
            log::error!("SP: ✓ 使用配置的service_name: {}", config_service_name);
            return config_service_name.to_string();
        }
        log::error!("SP: ✗ 配置的service_name为空或默认值，继续检查其他来源");

        // 2. 尝试从Envoy node metadata中的WORKLOAD_NAME获取
        log::error!("SP: [步骤2] 检查node.metadata.WORKLOAD_NAME");
        match self.get_property(vec!["node", "metadata", "WORKLOAD_NAME"]) {
            Some(workload_name) => {
                match String::from_utf8(workload_name) {
                    Ok(workload_name_str) if !workload_name_str.is_empty() => {
                        log::error!("SP: ✓ 从node.metadata.WORKLOAD_NAME获取到service_name: {}", workload_name_str);
                        return workload_name_str;
                    }
                    Ok(_) => log::error!("SP: ✗ node.metadata.WORKLOAD_NAME为空"),
                    Err(e) => log::error!("SP: ✗ node.metadata.WORKLOAD_NAME UTF-8转换失败: {}", e),
                }
            }
            None => log::error!("SP: ✗ node.metadata.WORKLOAD_NAME未找到"),
        }

        // 3. 尝试从Envoy node metadata中的app标签获取
        log::error!("SP: [步骤3] 检查node.metadata.app");
        match self.get_property(vec!["node", "metadata", "app"]) {
            Some(app_name) => {
                match String::from_utf8(app_name) {
                    Ok(app_name_str) if !app_name_str.is_empty() => {
                        log::error!("SP: ✓ 从node.metadata.app获取到service_name: {}", app_name_str);
                        return app_name_str;
                    }
                    Ok(_) => log::error!("SP: ✗ node.metadata.app为空"),
                    Err(e) => log::error!("SP: ✗ node.metadata.app UTF-8转换失败: {}", e),
                }
            }
            None => log::error!("SP: ✗ node.metadata.app未找到"),
        }

        // 4. 尝试从Envoy node metadata中的NAME获取
        log::error!("SP: [步骤4] 检查node.metadata.NAME");
        match self.get_property(vec!["node", "metadata", "NAME"]) {
            Some(node_name) => {
                match String::from_utf8(node_name) {
                    Ok(node_name_str) if !node_name_str.is_empty() => {
                        // 从node name中提取服务名（通常格式为service-name.namespace）
                        let service_name = match node_name_str.split('.').next() {
                            Some(name) if !name.is_empty() => name,
                            _ => &node_name_str,
                        };
                        log::error!("SP: ✓ 从node.metadata.NAME获取到service_name: {} -> {}", node_name_str, service_name);
                        return service_name.to_string();
                    }
                    Ok(_) => log::error!("SP: ✗ node.metadata.NAME为空"),
                    Err(e) => log::error!("SP: ✗ node.metadata.NAME UTF-8转换失败: {}", e),
                }
            }
            None => log::error!("SP: ✗ node.metadata.NAME未找到"),
        }

        // 5. 尝试从source workload name获取（对于入站流量）
        log::error!("SP: [步骤5] 检查source.workload.name");
        if let Some(source_workload) = self.get_property(vec!["source", "workload", "name"]) {
            if let Ok(source_workload_str) = String::from_utf8(source_workload) {
                if !source_workload_str.is_empty() {
                    log::error!("SP: ✓ 从source.workload.name获取到service_name: {}", source_workload_str);
                    return source_workload_str;
                }
            }
        }
        log::error!("SP: ✗ source.workload.name未找到或为空");

        // 6. 尝试从destination workload name获取（对于出站流量）
        log::error!("SP: [步骤6] 检查destination.workload.name");
        if let Some(dest_workload) = self.get_property(vec!["destination", "workload", "name"]) {
            if let Ok(dest_workload_str) = String::from_utf8(dest_workload) {
                if !dest_workload_str.is_empty() {
                    log::error!("SP: ✓ 从destination.workload.name获取到service_name: {}", dest_workload_str);
                    return dest_workload_str;
                }
            }
        }

        // 7. 尝试从destination service name获取
        log::error!("SP: 步骤7 - 检查destination.service.name");
        if let Some(dest_service) = self.get_property(vec!["destination", "service", "name"]) {
            if let Ok(dest_service_str) = String::from_utf8(dest_service) {
                if !dest_service_str.is_empty() {
                    log::error!("SP: ✓ 从destination.service.name获取到service_name: {}", dest_service_str);
                    return dest_service_str;
                } else {
                    log::error!("SP: ✗ destination.service.name为空");
                }
            } else {
                log::error!("SP: ✗ destination.service.name无法转换为字符串");
            }
        } else {
            log::error!("SP: ✗ 无法获取destination.service.name属性");
        }

        // 8. 尝试从cluster name中提取服务名
        log::error!("SP: 步骤8 - 检查cluster_name");
        if let Some(cluster_name) = self.get_property(vec!["cluster_name"]) {
            if let Ok(cluster_name_str) = String::from_utf8(cluster_name) {
                if !cluster_name_str.is_empty() {
                    log::error!("SP: cluster_name原始值: {}", cluster_name_str);
                    // cluster name通常格式为: outbound|port||service.namespace.svc.cluster.local
                    // 或者: inbound|port||
                    if let Some(service_part) = cluster_name_str.split('|').nth(3) {
                        if !service_part.is_empty() {
                            let service_name = match service_part.split('.').next() {
                                Some(name) if !name.is_empty() => name,
                                _ => service_part,
                            };
                            log::error!("SP: ✓ 从cluster_name获取到service_name: {} -> {}", cluster_name_str, service_name);
                            return service_name.to_string();
                        } else {
                            log::error!("SP: ✗ cluster_name第4部分为空");
                        }
                    } else {
                        log::error!("SP: ✗ cluster_name格式不符合预期，无法提取第4部分");
                    }
                } else {
                    log::error!("SP: ✗ cluster_name为空");
                }
            } else {
                log::error!("SP: ✗ cluster_name无法转换为字符串");
            }
        } else {
            log::error!("SP: ✗ 无法获取cluster_name属性");
        }

        // 9. 尝试从Kubernetes Service Account获取
        log::error!("SP: 步骤9 - 检查KUBERNETES_SERVICE_ACCOUNT环境变量");
        if let Some(service_account_bytes) = self.get_property(vec!["env", "KUBERNETES_SERVICE_ACCOUNT"]) {
            if let Ok(service_account) = String::from_utf8(service_account_bytes) {
                if !service_account.is_empty() && service_account != "default" {
                    log::error!("SP: ✓ 从KUBERNETES_SERVICE_ACCOUNT获取到service_name: {}", service_account);
                    return service_account;
                } else {
                    log::error!("SP: ✗ KUBERNETES_SERVICE_ACCOUNT为空或为default: {}", service_account);
                }
            }
        } else {
            log::error!("SP: ✗ 无法获取KUBERNETES_SERVICE_ACCOUNT环境变量");
        }

        // 10. 尝试从Pod名称获取（HOSTNAME通常等于Pod名称）
        log::error!("SP: 步骤10 - 检查HOSTNAME环境变量");
        if let Some(hostname_bytes) = self.get_property(vec!["env", "HOSTNAME"]) {
            if let Ok(hostname) = String::from_utf8(hostname_bytes) {
                if !hostname.is_empty() {
                    // 从Pod名称中提取服务名（去掉随机后缀）
                    let service_name = Self::extract_service_name_from_pod(&hostname);
                    log::error!("SP: ✓ 从HOSTNAME获取到service_name: {} -> {}", hostname, service_name);
                    return service_name;
                } else {
                    log::error!("SP: ✗ HOSTNAME为空");
                }
            }
        } else {
            log::error!("SP: ✗ 无法获取HOSTNAME环境变量");
        }

        // 11. 尝试从POD_NAME环境变量获取
        log::error!("SP: 步骤11 - 检查POD_NAME环境变量");
        if let Some(pod_name_bytes) = self.get_property(vec!["env", "POD_NAME"]) {
            if let Ok(pod_name) = String::from_utf8(pod_name_bytes) {
                if !pod_name.is_empty() {
                    let service_name = Self::extract_service_name_from_pod(&pod_name);
                    log::error!("SP: ✓ 从POD_NAME获取到service_name: {} -> {}", pod_name, service_name);
                    return service_name;
                } else {
                    log::error!("SP: ✗ POD_NAME为空");
                }
            }
        } else {
            log::error!("SP: ✗ 无法获取POD_NAME环境变量");
        }

        // 12. 尝试从特定的服务标识请求头中获取当前服务信息
        log::error!("SP: 步骤12 - 检查特定的服务标识请求头");
        // 这些头通常由服务网格或应用程序设置来标识当前服务
        let current_service_headers = vec!["x-service-name", "x-current-service", "service-name", "current-service"];
        for header_name in current_service_headers {
            if let Some(header_value) = self.get_http_request_header(header_name) {
                if !header_value.is_empty() {
                    log::error!("SP: ✓ 从{}头获取到当前service_name: {}", header_name, header_value);
                    return header_value;
                }
            }
        }
        
        // 注意：不使用:authority和host头，因为它们通常指向目标服务而不是当前服务
        log::error!("SP: ✗ 无法从服务标识请求头获取当前服务名");

        // 13. 尝试从路由和集群元数据获取
        log::error!("SP: 步骤13 - 检查路由和集群元数据");
        if let Some(route_name) = self.get_property(vec!["route_name"]) {
            if let Ok(route_str) = String::from_utf8(route_name) {
                if !route_str.is_empty() {
                    log::error!("SP: ✓ 从route_name获取到service_name: {}", route_str);
                    return route_str;
                }
            }
        }
        
        if let Some(virtual_cluster) = self.get_property(vec!["virtual_cluster_name"]) {
            if let Ok(cluster_str) = String::from_utf8(virtual_cluster) {
                if !cluster_str.is_empty() {
                    log::error!("SP: ✓ 从virtual_cluster_name获取到service_name: {}", cluster_str);
                    return cluster_str;
                }
            }
        }
        log::error!("SP: ✗ 无法从路由和集群元数据获取服务名");

        // 14. 尝试从Istio特定属性获取
        log::error!("SP: 步骤14 - 检查Istio特定属性");
        if let Some(canonical_service) = self.get_property(vec!["istio", "canonical_service"]) {
            if let Ok(service_str) = String::from_utf8(canonical_service) {
                if !service_str.is_empty() {
                    log::error!("SP: ✓ 从istio.canonical_service获取到service_name: {}", service_str);
                    return service_str;
                }
            }
        }
        
        if let Some(istio_workload) = self.get_property(vec!["istio", "workload_name"]) {
            if let Ok(workload_str) = String::from_utf8(istio_workload) {
                if !workload_str.is_empty() {
                    log::error!("SP: ✓ 从istio.workload_name获取到service_name: {}", workload_str);
                    return workload_str;
                }
            }
        }
        log::error!("SP: ✗ 无法从Istio特定属性获取服务名");

        // 15. 尝试从xDS配置获取
        log::error!("SP: 步骤15 - 检查xDS配置信息");
        if let Some(xds_cluster) = self.get_property(vec!["xds", "cluster_name"]) {
            if let Ok(cluster_str) = String::from_utf8(xds_cluster) {
                if !cluster_str.is_empty() {
                    log::error!("SP: xds.cluster_name原始值: {}", cluster_str);
                    // 对于inbound cluster (格式: inbound|port||)，尝试从其他地方获取服务名
                    if cluster_str.starts_with("inbound|") {
                        log::error!("SP: 检测到inbound流量，尝试从其他属性获取服务名");
                        // 继续到下一步，不直接返回
                    } else {
                        // 对于outbound cluster，尝试解析服务名
                        if let Some(service_part) = cluster_str.split('|').nth(3) {
                            if !service_part.is_empty() {
                                let service_name = match service_part.split('.').next() {
                                    Some(name) if !name.is_empty() => name,
                                    _ => service_part,
                                };
                                log::error!("SP: ✓ 从xds.cluster_name获取到service_name: {} -> {}", cluster_str, service_name);
                                return service_name.to_string();
                            }
                        }
                    }
        ...
        // 对于inbound流量，尝试从listener metadata获取服务名
        if let Some(listener_name) = self.get_property(vec!["listener_direction"]) {
            if let Ok(direction_str) = String::from_utf8(listener_name) {
                log::error!("SP: listener_direction: {}", direction_str);
                if direction_str == "inbound" {
                    // 尝试从listener metadata获取服务信息
                    if let Some(listener_metadata) = self.get_property(vec!["listener_metadata", "service_name"]) {
                        if let Ok(service_str) = String::from_utf8(listener_metadata) {
                            if !service_str.is_empty() {
                                log::error!("SP: ✓ 从listener_metadata.service_name获取到service_name: {}", service_str);
                                return service_str;
                            }
                        }
                    }
                    
                    // 尝试从workload metadata获取
                    if let Some(workload_name) = self.get_property(vec!["metadata", "workload_name"]) {
                        if let Ok(workload_str) = String::from_utf8(workload_name) {
                            if !workload_str.is_empty() {
                                log::error!("SP: ✓ 从metadata.workload_name获取到service_name: {}", workload_str);
                                return workload_str;
                            }
                        }
                    }
        ...
        log::error!("SP: ✗ 无法从xDS配置获取服务名");

        // 16. 尝试从环境相关属性获取
        log::error!("SP: 步骤16 - 检查环境相关属性");
        if let Some(deployment_name) = self.get_property(vec!["environment", "deployment_name"]) {
            if let Ok(deploy_str) = String::from_utf8(deployment_name) {
                if !deploy_str.is_empty() {
                    log::error!("SP: ✓ 从environment.deployment_name获取到service_name: {}", deploy_str);
                    return deploy_str;
                }
            }
        }
        
        if let Some(env_pod_name) = self.get_property(vec!["environment", "pod_name"]) {
            if let Ok(pod_str) = String::from_utf8(env_pod_name) {
                if !pod_str.is_empty() {
                    let service_name = Self::extract_service_name_from_pod(&pod_str);
                    log::error!("SP: ✓ 从environment.pod_name提取到service_name: {}", service_name);
                    return service_name;
                }
            }
        }
        log::error!("SP: ✗ 无法从环境相关属性获取服务名");

        // 17. 尝试从其他Kubernetes环境变量获取
        log::error!("SP: 步骤17 - 检查SERVICE_NAME环境变量");
        // 注意：在WASM环境中，std::env::var可能不可用，使用Envoy属性替代
        if let Some(service_name_bytes) = self.get_property(vec!["env", "SERVICE_NAME"]) {
            if let Ok(service_name) = String::from_utf8(service_name_bytes) {
                if !service_name.is_empty() {
                    log::error!("SP: ✓ 从SERVICE_NAME获取到service_name: {}", service_name);
                    return service_name;
                } else {
                    log::error!("SP: ✗ SERVICE_NAME为空");
                }
            }
        } else {
            log::error!("SP: ✗ 无法获取SERVICE_NAME环境变量");
        }

        // 18. 对于inbound流量，尝试从deployment和pod labels获取服务名
        log::error!("SP: 步骤18 - 检查deployment和pod labels");
        
        // 尝试从app label获取
        if let Some(app_label) = self.get_property(vec!["metadata", "labels", "app"]) {
            if let Ok(app_str) = String::from_utf8(app_label) {
                if !app_str.is_empty() {
                    log::error!("SP: ✓ 从metadata.labels.app获取到service_name: {}", app_str);
                    return app_str;
                }
            }
        }
        
        // 尝试从app.kubernetes.io/name label获取
        if let Some(app_name_label) = self.get_property(vec!["metadata", "labels", "app.kubernetes.io/name"]) {
            if let Ok(app_name_str) = String::from_utf8(app_name_label) {
                if !app_name_str.is_empty() {
                    log::error!("SP: ✓ 从metadata.labels.app.kubernetes.io/name获取到service_name: {}", app_name_str);
                    return app_name_str;
                }
            }
        }
        ...
        // 尝试从service label获取
        if let Some(service_label) = self.get_property(vec!["metadata", "labels", "service"]) {
            if let Ok(service_str) = String::from_utf8(service_label) {
                if !service_str.is_empty() {
                    log::error!("SP: ✓ 从metadata.labels.service获取到service_name: {}", service_str);
                    return service_str;
                }
            }
        }
        
        log::error!("SP: ✗ 无法从deployment和pod labels获取服务名");

        // 19. 最后使用配置文件中的默认值
        log::warn!("SP: ⚠️ 所有检测方法都失败，使用默认配置值: {}", config_service_name);
        config_service_name.to_string()
    }

    /// 从Pod名称中提取服务名称
    /// Pod名称通常格式为: service-name-deployment-hash-pod-hash
    /// 或者: service-name-hash
    fn extract_service_name_from_pod(pod_name: &str) -> String {
        // 移除常见的Kubernetes后缀模式
        let name = pod_name
            // 移除ReplicaSet hash (格式: name-hash-hash)
            .split('-')
            .collect::<Vec<&str>>();
        
        if name.len() >= 3 {
            // 如果有3个或更多部分，通常前面的部分是服务名
            // 例如: productpage-v1-6b746f74dc-7hxgn -> productpage-v1
            let mut service_parts = Vec::new();
            for (i, part) in name.iter().enumerate() {
                // 如果遇到看起来像hash的部分（全是字母数字且长度>=5），就停止
                if part.len() >= 5 && part.chars().all(|c| c.is_ascii_alphanumeric()) {
                    // 检查是否是典型的Kubernetes hash
                    if i >= 1 {
                        break;
                    }
                }
                service_parts.push(*part);
            }
            
            if !service_parts.is_empty() {
                return service_parts.join("-");
            }
        }
    ...
    /// 打印所有环境变量用于调试
    fn log_all_environment_variables(&self) {
        log::error!("SP: ========== 环境变量列表 ==========");
        
        // 常见的Kubernetes和Istio环境变量
        let env_vars = vec![
            "HOSTNAME", "POD_NAME", "POD_NAMESPACE", "POD_IP", "NODE_NAME",
            "SERVICE_ACCOUNT", "KUBERNETES_SERVICE_ACCOUNT", "SERVICE_NAME",
            "ISTIO_META_WORKLOAD_NAME", "ISTIO_META_APP", "ISTIO_META_VERSION",
            "ISTIO_META_CLUSTER_ID", "ISTIO_META_MESH_ID", "ISTIO_META_NAMESPACE",
            "PILOT_ENABLE_WORKLOAD_ENTRY_AUTOREGISTRATION", "PILOT_ENABLE_CROSS_CLUSTER_WORKLOAD_ENTRY",
            "ISTIO_PROXY_VERSION", "ISTIO_VERSION", "SIDECAR_STATUS",
        ];
        
        for var in env_vars {
            match std::env::var(var) {
                Ok(value) => log::error!("SP: ENV[{}] = '{}'", var, value),
                Err(_) => log::error!("SP: ENV[{}] = <未设置>", var),
            }
        }
        
        log::error!("SP: ========== 环境变量列表结束 ==========");
    }

    /// 打印所有可用的Envoy属性用于调试
    fn log_all_envoy_attributes(&self) {
        log::error!("SP: ========== Envoy属性列表 ==========");
        ...
        let envoy_attrs = vec![
            ...
            vec!["node", "id"],
            vec!["node", "cluster"],
            vec!["node", "metadata", "NAME"],
            vec!["node", "metadata", "NAMESPACE"],
            vec!["node", "metadata", "WORKLOAD_NAME"],
            vec!["node", "metadata", "app"],
            vec!["node", "metadata", "version"],
            vec!["node", "metadata", "CLUSTER_ID"],
            vec!["node", "metadata", "MESH_ID"],
            vec!["node", "metadata", "LABELS"],
            vec!["node", "metadata", "OWNER"],
            vec!["node", "metadata", "SERVICE_ACCOUNT"],
            vec!["node", "metadata", "ISTIO_VERSION"],
            vec!["node", "metadata", "PILOT_ENABLE_WORKLOAD_ENTRY_AUTOREGISTRATION"],
            
            // Source相关
            vec!["source", "address"],
            vec!["source", "port"],
            vec!["source", "workload", "name"],
            vec!["source", "workload", "namespace"],
            vec!["source", "service", "name"],
            vec!["source", "service", "namespace"],
            vec!["source", "labels"],
            vec!["source", "principal"],
            vec!["source", "certificate_fingerprint"],
            
            // Destination相关
            vec!["destination", "address"],
            vec!["destination", "port"],
            vec!["destination", "service", "name"],
            vec!["destination", "service", "namespace"],
            vec!["destination", "workload", "name"],
            vec!["destination", "workload", "namespace"],
            vec!["destination", "labels"],
            vec!["destination", "principal"],
            vec!["destination", "certificate_fingerprint"],
            
            // Request相关
            vec!["request", "headers"],
            vec!["request", "method"],
            vec!["request", "path"],
            vec!["request", "url_path"],
            vec!["request", "host"],
            vec!["request", "scheme"],
            vec!["request", "protocol"],
            vec!["request", "id"],
            vec!["request", "time"],
            vec!["request", "useragent"],
            vec!["request", "referer"],
            
            // Response相关
            vec!["response", "code"],
            vec!["response", "headers"],
            vec!["response", "flags"],
            vec!["response", "grpc_status"],
            
            // Connection相关
            vec!["connection", "id"],
            vec!["connection", "mtls"],
            vec!["connection", "requested_server_name"],
            vec!["connection", "tls_version"],
            vec!["connection", "subject_local_certificate"],
            vec!["connection", "subject_peer_certificate"],
            
            // 路由和集群相关
            vec!["cluster_name"],
            vec!["cluster_metadata"],
            vec!["route_name"],
            vec!["route_metadata"],
            vec!["virtual_cluster_name"],
            
            // Upstream相关
            vec!["upstream", "address"],
            vec!["upstream", "port"],
            vec!["upstream", "tls_version"],
            vec!["upstream", "subject_local_certificate"],
            vec!["upstream", "subject_peer_certificate"],
            vec!["upstream", "local_address"],
            vec!["upstream", "transport_failure_reason"],
            
            // Listener相关
            vec!["listener_direction"],
            vec!["listener_metadata"],
            
            // Filter相关
            vec!["filter_chain_name"],
            vec!["filter_state"],
            
            // Istio特定属性
            vec!["istio", "canonical_service"],
            vec!["istio", "canonical_revision"],
            vec!["istio", "workload_name"],
            vec!["istio", "namespace"],
            
            // xDS相关
            vec!["xds", "cluster_name"],
            vec!["xds", "route_name"],
            vec!["xds", "listener_name"],
            
            // 环境相关
            vec!["environment", "deployment_name"],
            vec!["environment", "pod_name"],
            vec!["environment", "namespace"],
            
            // 其他可能的属性
            vec!["metadata"],
            vec!["dynamic_metadata"],
            vec!["typed_filter_metadata"],
        ];
        
        for attr_path in envoy_attrs {
            let path_str = attr_path.join(".");
            // 使用安全的方式获取属性，避免panic
            match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                self.get_property(attr_path.clone())
            })) {
                Ok(Some(value)) => {
                    match String::from_utf8(value.clone()) {
                        Ok(str_value) => log::error!("SP: ENVOY[{}] = '{}'", path_str, str_value),
                        Err(_) => log::error!("SP: ENVOY[{}] = <二进制数据，长度: {} bytes>", path_str, value.len()),
                    }
                },
                Ok(None) => log::error!("SP: ENVOY[{}] = <未找到>", path_str),
                Err(_) => log::error!("SP: ENVOY[{}] = <获取时发生错误>", path_str),
            }
        }
        
        log::error!("SP: ========== Envoy属性列表结束 ==========");
    }
...
impl HttpContext for SpHttpContext {
    fn on_http_request_headers(&mut self, _num_headers: usize, end_of_stream: bool) -> Action {
        ...
        // 在这里进行service_name的动态检测和更新
        let detected_service_name = self.detect_service_name(&self.config.service_name);
        ...
        // Update span builder with trace context, session ID and detected service name
