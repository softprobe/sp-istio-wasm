...
echo "SP-Istio Injection Cache Test"
echo "============================"
...
print_status "Testing injection cache with: curl -X POST \"$TEST_URL\" ${HEADERS[*]} -d \"$DATA\""
...
print_status "Waiting 3 seconds for cache storage to complete..."
sleep 3
...
if [ "$TRACE_ID1" = "$TRACE_ID2" ]; then
    ...
    print_success "✓ This indicates the response was served from injection cache"
else
...
print_status "2. Second request: Cache hit → /v1/inject returns cached data → Same trace ID returned"
print_status ""
