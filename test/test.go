...
func main() {
    ...
    q.Set("startTime", testStart)
    q.Set("limit", "50")
    tracesEndpoint := fmt.Sprintf("%s/v1/traces?%s", strings.TrimRight(backendURL, "/"), q.Encode())
    sessionURL := fmt.Sprintf("%s/api/trace/sessions/%s/traces", strings.TrimRight(backendURL, "/"), url.PathEscape(sessionID))
